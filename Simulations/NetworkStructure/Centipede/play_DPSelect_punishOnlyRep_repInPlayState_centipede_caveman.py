...
def play_dp_s_play_punish_only_rep_rep_in_play_state_centipede_caveman(num_eps: int, max_num_rounds: int, G: nx.Graph,
    ...
    for episode in tqdm(range(num_eps)):
        ...
        for agent_1_idx, agent_1 in enumerate(population):
            ...
            select_exp_1 = (selection_states[agent_1_idx], possible_partner_idx, running_reward_1, new_select_state_1,
                            is_terminal_round)
    ...
    if logging:
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'Logs',
                               'play_dp_s_play_punish_only_rep_rep_in_play_state_centipede_caveman.json'), 'w+', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    mean_combined_sum_reward, agent_stats = play_dp_s_play_punish_only_rep_rep_in_play_state_centipede_caveman(
        1000, 10, G,
        population)
