...
def play_costly_punish_ttp_play_punish_rep_rep_in_play_state(cooperation_dilemma: MatrixGame, num_eps: int, num_rounds: int,
    ...
    if logging:
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'Logs',
                               'play_ttp_play_punish_rep_rep_in_play_state.json'), 'w+', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    population = [PunishNoSelectAgent(play_stage_model_parameters, punish_stage_model_parameters) for _ in
                  range(NUM_AGENTS)]

    start = time.perf_counter()
    mean_combined_sum_reward, agent_stats = play_costly_punish_ttp_play_punish_rep_rep_in_play_state(cooperation_game, 1000,
                                                                                       10, population)
