...
from play_Costly_Punish_TPPOnly_playPunishRep_repInPlayState_pd import play_costly_punish_ttp_play_punish_rep_rep_in_play_state
...
if __name__ == "__main__":
    game_name = "play_costly_punish_ttp_play_punish_rep_rep_in_play_state"
    run_stats = []
    run_mean_combined_reward = []
    for run in tqdm(range(NUM_RUNS)):
        population = [PunishNoSelectAgent(play_stage_model_parameters, punish_stage_model_parameters)
                      for _ in range(NUM_AGENTS)]
        mean_combined_sum_reward, agent_stats = play_costly_punish_ttp_play_punish_rep_rep_in_play_state(cooperation_game,
                                                                                           NUM_EPISODES,
