# Reputation impacted by punishing behaviour only. Playing and punishing actions also depend on reputation (play and punish state).
...
def play_ttp_s_play_punish_only_rep_rep_in_play_punish_state(num_eps: int, max_num_rounds: int,
    ...
    if logging:
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'Logs',
                               'play_ttp_s_play_punish_only_rep_rep_in_play_punish_state.json'), 'w+', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    mean_combined_sum_reward, agent_stats = play_ttp_s_play_punish_only_rep_rep_in_play_punish_state(10, 2, population)
