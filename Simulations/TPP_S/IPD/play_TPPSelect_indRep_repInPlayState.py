...
from Mechanisms.Reputation import societal_reputation_update, avenger_reputation, persecutor_reputation, \
    cooperation_reputation, defection_reputation
from Mechanisms.GamePlay import choose_play_actions

societal_just_punish_reputation = 2
societal_unjust_punish_reputation = -3
societal_no_punish_reputation = 0

def play_ttp_s_ind_rep_rep_in_play_state(cooperation_dilemma: MatrixGame, num_eps: int, num_rounds: int,
    ...
    for episode in tqdm(range(num_eps)):
        ...
        play_pairings = select_ind_rep_partners(population, select_state)
        for agent_1_idx, agent_1 in enumerate(population):
            ...
            if logging:
                ...
                if len(agent_stats[agent_2_idx]["select_history"]) > 0:
                    agent_stats[agent_1_idx]["prev_play_select"].append((agent_2_idx, agent_stats[agent_2_idx]["select_history"][-1]))
                agent_stats[agent_1_idx]["prev_reputation_select"].append((agent_2_idx, select_state[agent_1_idx][agent_2_idx]))
                flattened_agent_2_punish_history = [punish for episode in agent_stats[agent_2_idx]["punish_history"] for punish in episode]
                if len(flattened_agent_2_punish_history) > 0:
                    agent_stats[agent_1_idx]["prev_punish_select"].append((agent_2_idx, flattened_agent_2_punish_history[-1]))
                if len(agent_stats[agent_2_idx]["punish_justly"]) > 0:
                    agent_stats[agent_1_idx]["prev_punish_justly_select"].append((agent_2_idx, agent_stats[agent_2_idx]["punish_justly"][-1]))
                if len(agent_stats[agent_2_idx]["punish_unjustly"]) > 0:
                    agent_stats[agent_1_idx]["prev_punish_unjustly_select"].append((agent_2_idx, agent_stats[agent_2_idx]["punish_unjustly"][-1]))
            is_terminal_round = 0
            for round_idx in range(num_rounds):
                ...
                play_state_1 = [agent_prev_play_actions[agent_2_idx], agent_prev_play_actions[agent_1_idx],
                                agent_reputations[agent_1_idx][agent_2_idx], agent_reputations[agent_1_idx][agent_1_idx]]
                play_state_2 = [agent_prev_play_actions[agent_1_idx], agent_prev_play_actions[agent_2_idx],
                                agent_reputations[agent_2_idx][agent_1_idx], agent_reputations[agent_2_idx][agent_2_idx]]
                ...
                # Initially, reputations will also be affected by play action.
                if play_dict[play_action_idx_1] == "C":
                    agent_reputations = societal_reputation_update(agent_reputations, agent_1_idx, cooperation_reputation)
                else:
                    agent_reputations = societal_reputation_update(agent_reputations, agent_1_idx, defection_reputation)

                if play_dict[play_action_idx_2] == "C":
                    agent_reputations = societal_reputation_update(agent_reputations, agent_2_idx, cooperation_reputation)
                else:
                    agent_reputations = societal_reputation_update(agent_reputations, agent_2_idx, defection_reputation)
                ...
                select_exp_1 = (select_state[agent_1_idx], possible_partner_idx, play_reward_1, new_select_state_1[agent_1_idx], is_terminal_round)
                agent_1.select_model.replay_buffer.store(*select_exp_1)
                agent_1.select_model.optimise_model()

                new_play_state_1 = [play_action_idx_2, play_action_idx_1, agent_reputations[agent_1_idx][agent_2_idx],
                                    agent_reputations[agent_1_idx][agent_2_idx]]
                ...
                new_play_state_2 = [play_action_idx_1, play_action_idx_2, agent_reputations[agent_2_idx][agent_1_idx],
                                    agent_reputations[agent_2_idx][agent_2_idx]]
    ...
    if logging:
        with open("play_ttp_s_ind_rep_rep_in_play_state.json", 'w+', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    mean_combined_sum_reward, agent_stats = play_ttp_s_ind_rep_rep_in_play_state(cooperation_game, 1000, 10, population)
