# Reputation impacted by playing behaviour only. Play actions also depend on reputation (play state).
...
def play_dp_s_play_play_only_rep_rep_in_play_state_centipede(num_eps: int, max_num_rounds: int,
    ...
    if logging:
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'Logs',
                               'play_dp_s_play_play_only_rep_rep_in_play_state_centipede.json'), 'w+', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    mean_combined_sum_reward, agent_stats = play_dp_s_play_play_only_rep_rep_in_play_state_centipede(10, 2, population)
