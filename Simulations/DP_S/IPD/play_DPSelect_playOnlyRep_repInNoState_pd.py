# Reputation impacted by both playing behaviour.
...
def play_dp_s_play_only_rep_rep_in_no_state(cooperation_dilemma: MatrixGame, num_eps: int, num_rounds: int,
    ...
    if logging:
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'Logs', 'play_dp_s_play_only_rep_rep_in_no_state.json'), 'w+', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    mean_combined_sum_reward, agent_stats = play_dp_s_play_only_rep_rep_in_no_state(cooperation_game, 1000,
                                                                                      10, population)
